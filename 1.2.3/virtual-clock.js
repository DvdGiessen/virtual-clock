          ...
          if (this._running && this._rate !== 0 && time >= this._minimum && time <= this._maximum) {
            ...
            if (currentTime === lastCalled) {
              ...
              if (until >= 0 || this._loop && this._minimum > -Infinity && this._maximum < Infinity) {
                ...
                until = Math.ceil(until); // Workaround: many common JavaScript engines internally use a 32-bit signed integer
                ...
                if (until > 0x7fffffff) {
                  var untilRecalculate = Math.min(Math.ceil(until / 2), 0x7fffffff);
                  newTimeoutID = setTimeout(function () {
                    return _this2._recalculateTimeListener(listener);
                  }, untilRecalculate);
                } else {
                  newTimeoutID = setTimeout(function () {
